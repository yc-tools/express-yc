...
export type TerraformMode = 'serverless' | 'containers' | 'instance-group';
...
export async function prepareTerraformProject(mode: TerraformMode): Promise<string> {
  const templateDir = path.join(TERRAFORM_TEMPLATE_DIR, mode);
  if (!(await fs.pathExists(templateDir))) {
    throw new Error(`Embedded terraform template not found: ${templateDir}`);
  }

  const workingDir = await fs.mkdtemp(path.join(os.tmpdir(), `express-yc-terraform-${mode}-`));
  await fs.copy(templateDir, workingDir);
  return workingDir;
}
...
export class TerraformRunner {
  ...
  async listState(env?: NodeJS.ProcessEnv): Promise<string[]> {
    try {
      const { stdout } = await this.run(['state', 'list'], { captureOutput: true, env });
      return stdout
        .split('\n')
        .map((line) => line.trim())
